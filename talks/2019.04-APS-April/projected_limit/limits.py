...
class LimitFigure:
    ...
    def add_experiment(self, name):
        ...
        if name=='grand_10k':
            ...
            # self.ax.plot(energy, flux,
            #              color='#2288AA')
            energy, flux, _, _ = self.get_data('sensitivities/ara_projected_eff.txt')
            self.ax.plot(energy, flux,
            ...
            if self.e_power==1:
                # self.ax.annotate('ARA (2x1yr)',
                #                  xy=(2.5e8, 6e-15*self.e_bins), xycoords='data',
...
# efficiencies = np.array([0.159996783, 0.250440134, 0.342262861, 0.418965276, 0.530719322, 0.590234764, 0.607830508, 0.617866097])
efficiencies = np.array([0.1651065469384487, 0.2561069205964717, 0.3150219082385102, 0.38968686833848015, 0.4667815514643482, 0.5405751374009642, 0.568616919619615, 0.5977011822981313])

ara_available = (851*ara_100m + 3351*ara_200m + 306*ara_200mpa)/365.25

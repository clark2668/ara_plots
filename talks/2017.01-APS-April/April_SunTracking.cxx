...
int ThetaCorrect(double radius, double depth, int input_theta);
...
int main(int argc, char **argv)
{
	...
	double select_radius = 30.; //make a map with a 30m punitive source direction
	...
	TProfile *phiProfile = new TProfile("phi","phi",100,0,60,-80,-35);
	TProfile *thetaProfile = new TProfile("theta","theta",100,0,60,-0,70);
	...
	int num_in_time_window=0;
 	...
 	for(Int_t event=0;event<numEntries;event++) { //loop over all events
		...
		if((realIcrrEvPtr->isCalPulserEvent() == true) || (rawIcrrEvPtr -> trig.trigType ==68)) num_not_RF++;
		...
		if( //2==2
			...
			int temp_theta = ThetaCorrect(3000., 18.9443, PeakTheta_value);
			PeakTheta_value=temp_theta;
			peakTheta.push_back((double) PeakTheta_value);
			peakPhi.push_back((double) peak_phi_corrected );
			
			phiProfile->Fill((double) min + ((double)(hour-2))*60., peak_phi_corrected);
			thetaProfile->Fill((double) min + ((double)(hour-2))*60., PeakTheta_value);
			...
			double this_event_theta = 90.-AraGeomTool::getSunZenithLongLat(station_long, station_lat, unixtime_1);
	...
	cout<<"Num possible is "<<numEntries<<endl;
	cout<<"Num not RF is"<<num_not_RF<<endl;
     
	TGraph *sunPhi = new TGraph(num_found, &eventTime[0], &peakPhi[0]);
	TGraph *reportedsunPhi = new TGraph(num_found, &eventTime[0], &reportedpeakPhi[0]);
	TCanvas *tracking = new TCanvas("","",1600,900);
	sunPhi->Draw("AP");
		sunPhi->GetYaxis()->SetRangeUser(-75.,-35.);
		sunPhi->GetXaxis()->SetRangeUser(-10.,55);
		sunPhi->SetMarkerStyle(kFullDotLarge);
		tracking->Update();
		sunPhi->GetXaxis()->SetTitle("Minutes after 2 GMT");
		sunPhi->GetYaxis()->SetTitle("Phi (deg)");
		//sunPhi->GetYaxis()->SetTitleOffset(1.5);
		sunPhi->GetYaxis()->SetLabelSize(0.05);
		sunPhi->GetXaxis()->SetLabelSize(0.05);
		sunPhi->GetYaxis()->SetTitleSize(0.05);
		sunPhi->GetXaxis()->SetTitleSize(0.05);
		sunPhi->SetLineColor(kBlack);
		sunPhi->SetTitle("");
		//sunPhi->SetTitle("Location of Reconstruction Peak in Phi");
		tracking->Update();
	reportedsunPhi->Draw("same"); //draw this on the same canvas, with a line
		reportedsunPhi->SetLineColor(kRed);
		reportedsunPhi->SetLineWidth(3);
		tracking->Update();
	char save_title[150];
	sprintf(save_title,"/home/clark.2668/workspace/TrunkSolarFlares/results/today/April_SunTracking_Phi.pdf",year_now,month_now,day_now);
	tracking->SaveAs(save_title);
	delete sunPhi;
	delete tracking;
     
	TGraph *sunTheta = new TGraph(num_found, &eventTime[0], &peakTheta[0]);
	TGraph *reportedsunTheta = new TGraph(num_found, &eventTime[0], &reportedpeakTheta[0]);
	TCanvas *tracking_2 = new TCanvas("","",1600,900);
	sunTheta->Draw("AP");
		sunTheta->GetYaxis()->SetRangeUser(0.,40.);
		sunTheta->GetXaxis()->SetRangeUser(-10.,55.);
		sunTheta->SetMarkerStyle(kFullDotLarge);
		tracking_2->Update();
		sunTheta->GetXaxis()->SetTitle("Minutes after 2 GMT");
		sunTheta->GetYaxis()->SetTitle("Theta (deg)");
		//sunTheta->GetYaxis()->SetTitleOffset(1.5);
		sunTheta->SetLineColor(kBlack);
		sunTheta->SetTitle("");
		sunTheta->GetYaxis()->SetLabelSize(0.05);
		sunTheta->GetXaxis()->SetLabelSize(0.05);
		sunTheta->GetYaxis()->SetTitleSize(0.05);
		sunTheta->GetXaxis()->SetTitleSize(0.05);
		//sunTheta->SetTitle("Location of Reconstruction Peak in Phi");
		tracking->Update();
	reportedsunTheta->Draw("same"); //draw this on the same canvas, with a line
		reportedsunTheta->SetLineColor(kRed);
		reportedsunTheta->SetLineWidth(3);
		tracking->Update();   
	TLegend *theta_legend = new TLegend(0.55,0.65,0.9,0.85);
		theta_legend->AddEntry(sunTheta,"Data","lp");
		theta_legend->AddEntry(reportedsunTheta,"Computed Sun Direction","lp");
		theta_legend->SetBorderSize(0);  //no border for legend                                                                                                                                                                                         
		theta_legend->SetFillColor(0);  //fill color is white                                                                                                                                                                                           
		//theta_legend->Draw();
	sprintf(save_title,"/home/clark.2668/workspace/TrunkSolarFlares/results/today/April_SunTracking_Theta.pdf",year_now,month_now,day_now);
	tracking_2->SaveAs(save_title);
	delete sunTheta;
	delete tracking_2;
	delete theta_legend;

	TCanvas *tracking3 = new TCanvas("","",1100,850);
	phiProfile->Draw();
		phiProfile->GetYaxis()->SetRangeUser(-85.,-35.);
		phiProfile->GetXaxis()->SetRangeUser(0.,55);
		tracking3->Update();
		phiProfile->GetXaxis()->SetTitle("Minutes after 2 GMT");
		phiProfile->GetYaxis()->SetTitle("Phi (deg)");
		//phiProfile->GetYaxis()->SetTitleOffset(1.5);
		phiProfile->SetLineColor(kBlack);
		phiProfile->SetTitle("");
		phiProfile->GetYaxis()->SetLabelSize(0.05);
		phiProfile->GetXaxis()->SetLabelSize(0.05);
		phiProfile->GetYaxis()->SetTitleSize(0.05);
		phiProfile->GetXaxis()->SetTitleSize(0.05);
		phiProfile->SetLineWidth(2);
		//sunPhi->SetTitle("Location of Reconstruction Peak in Phi");
		tracking3->Update();
	...
	reportedsunPhi->Draw("same"); //draw this on the same canvas, with a line
		...
		reportedsunPhi->SetLineWidth(3);
		tracking->Update();
	sprintf(save_title,"/home/clark.2668/workspace/TrunkSolarFlares/results/today/April_SunTracking_Phi_Profile.pdf",year_now,month_now,day_now);
	tracking3->SaveAs(save_title);
	delete reportedsunPhi;
	delete phiProfile;
	delete tracking3;
     
	TCanvas *tracking4 = new TCanvas("","",1100,850);
	thetaProfile->Draw();
		thetaProfile->GetYaxis()->SetRangeUser(0.,40.);
		thetaProfile->GetXaxis()->SetRangeUser(0.,60.);
		tracking4->Update();
		thetaProfile->GetXaxis()->SetTitle("Minutes after 2 GMT");
		thetaProfile->GetYaxis()->SetTitle("Theta (deg)");
		//thetaProfile->GetYaxis()->SetTitleOffset(1.5);
		thetaProfile->SetLineColor(kBlack);
		thetaProfile->SetLineWidth(2);
		thetaProfile->GetYaxis()->SetLabelSize(0.05);
		thetaProfile->GetXaxis()->SetLabelSize(0.05);
		thetaProfile->GetYaxis()->SetTitleSize(0.05);
		thetaProfile->GetXaxis()->SetTitleSize(0.05);
		thetaProfile->SetTitle("");
		//sunTheta->SetTitle("Location of Reconstruction Peak in Phi");
		tracking4->Update();
	reportedsunTheta->Draw("same"); //draw this on the same canvas, with a line
		reportedsunTheta->SetLineColor(kRed);
		reportedsunTheta->SetLineWidth(3);
		tracking4->Update();   
	TLegend *theta_legend2 = new TLegend(0.55,0.65,0.9,0.85);
		theta_legend2->AddEntry(thetaProfile,"Data","lp");
		theta_legend2->AddEntry(reportedsunTheta,"Computed Sun Direction","lp");
		theta_legend2->SetBorderSize(0);  //no border for legend                                                                                                                                                                                         
		theta_legend2->SetFillColor(0);  //fill color is white                                                                                                                                                                                           
		theta_legend2->Draw();
	sprintf(save_title,"/home/clark.2668/workspace/TrunkSolarFlares/results/today/April_SunTracking_Theta_Profile.pdf",year_now,month_now,day_now);
	tracking4->SaveAs(save_title);
	delete thetaProfile;
	delete reportedsunTheta;
	delete tracking4;
	delete theta_legend2;
...
//fixes the theta angle
int ThetaCorrect(double radius, double depth, int input_theta){
	
	int output_theta = 0;
	if(input_theta < 0){
		cout<<"This function only applies to above surface angles!"<<endl;
		return output_theta;
	}
	
	double BigPhi = 90. - (double) input_theta;
	//cout<<"BigPhi is "<<BigPhi<<endl;
	double Prime = 180. - (double) input_theta;
	//cout<<"Prime is "<<Prime<<endl;
	double R4 = depth * tan(BigPhi * TMath::DegToRad());
	//cout<<"R4 is "<<R4<<endl;
	double R1 = depth / cos(BigPhi* TMath::DegToRad());
	//cout<<"R1 is "<<R1<<endl;
	double R2 = radius - R1;
	//cout<<"R2 is "<<R2<<endl;
	double R3 = sqrt( (pow(R2,2)) + (pow(R4,2)) - (2*R2 * R4 * cos(Prime * TMath::DegToRad() )));
	//cout<<"R3 is "<<R3<<endl;
	double phi = acos( (pow(R2,2)-pow(R3,2)-pow(R4,2)) / (-2*R3*R4) );
	output_theta = (int) (phi * TMath::RadToDeg());
	//cout<<"Output theta "<<output_theta<<endl;
	return output_theta;
}

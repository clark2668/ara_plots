...
////
////    This combines the ICL events in spectra, then FFT's them to waveforms
////////////////////////////////////////////////////////////////////////////////
...
const int n_chan = 16; //number of channels to make templates for
const int num_wanted_for_template = 10; //number of waveforms to go into the template
...
bool isICLPulserEvent(RawIcrrStationEvent *evPtr);
void averageTemplates(TGraph *in[n_chan][num_wanted_for_template], TGraph *out[n_chan]);
void AverageSpectra(Int_t numGraphs, TGraph **grPtr, TGraph * (&OutputGraphs)[2], vector<double>& incident_angles, bool apply_fresnel, int pol);
TGraph *DivideGraphs(TGraph *grNum, TGraph *grDen);
int plot_mapping(int input);
void spectra_ratio_mapping(int input, int & output1, int & output2);
void Brian_GetFresnel_spol(double i_ang, double n1, double n2, double &r, double &t);
void Brian_GetFresnel_ppol(double i_ang, double n1, double n2, double &r, double &t);
void simpleVoltageSpectrumwithFresnel(TGraph *grPtr, TGraph * (&OutputGraphs)[2], double incident_angle, bool apply_fresnel, int pol);
TGraph *applySpectra(TGraph *input, TGraph *response);
TGraph* simpleVoltageSpectrum(TGraph *grPtr);
...
/* Locations of antennas
 ...
 Ant 0 (X, Y, Z) = -8.41937 , -4.40393 , -20.4981
 Ant 1 (X, Y, Z) = 3.02254 , 10.4089 , -30.5589
 Ant 2 (X, Y, Z) = -0.423236 , -11.1265 , -22.5073
 Ant 3 (X, Y, Z) = -8.41934 , -4.40393 , -25.4981
 Ant 4 (X, Y, Z) = 9.22197 , -6.15202 , -27.7292
 Ant 5 (X, Y, Z) = -0.423215 , -11.1265 , -27.5073
 Ant 6 (X, Y, Z) = 3.02252 , 10.4089 , -25.5588
 Ant 7 (X, Y, Z) = 9.22197 , -6.15202 , -22.7292
 Ant 8 (X, Y, Z) = -2.48131 , -1.74931 , -1.21484
 Ant 9 (X, Y, Z) = 4.39363 , -2.40897 , -1.19102
 Ant 10 (X, Y, Z) = 1.58058 , 3.80025 , -1.19102
 Ant 11 (X, Y, Z) = -2.48131 , -1.74931 , -2.21484
 Ant 12 (X, Y, Z) = -9.07493 , 3.85737 , -26.4111
 Ant 13 (X, Y, Z) = -9.07493 , 3.85737 , -30.4111
 Ant 14 (X, Y, Z) = 0 , 0 , 0
 Ant 15 (X, Y, Z) = 0 , 0 , 0
...
int main(int argc, char **argv)
{
	if(argc<2) {  // Check to make sure there are enough arguments to do something meaningful
		std::cout << "Usage requires two input files! "<< std::endl;
		return -1;
	}
	gStyle->SetOptStat(0);                                                                                                                                                                    
	...
	double select_radius = 3000.; //make a map with a 3000m punitive source direction
	...
	int antenna_1_number=2; //the antenna number of the first antenna you are interested in correlating
	const int num_antennas_used=8;

	
	Double_t antenna_positions [num_antennas_used][3]; //array for the positions of the antenas; the first index demarcates the antenna, the second index demarcates the positions 
	Double_t  antenna_rho[num_antennas_used]; //eight entry array for the antenna rhos; one entry for each antenna
	Double_t  antenna_phi[num_antennas_used]; //eight entry array for the antenna phis; one entry for each antenna
	Double_t antenna_pol[num_antennas_used]; //eight entry array for the anntena polarizations; one for each antenna
	
	for(int antenna_iter=0; antenna_iter<num_antennas_used; antenna_iter++){
	  	antenna_pol[antenna_iter]=( (int) geomTool->getStationInfo(0)->getAntennaInfo(antenna_iter)->polType);
		
		Double_t antenna_position_temporary_holder[3]; //temporary variable for the antenna locations
		Double_t antenna_rho_holder=0; //temporary variable for the antenna rhos
		Double_t antenna_phi_holder=0; //temporary variable for the antenna phis
		fillAntennaPositions(antenna_iter, antenna_position_temporary_holder, antenna_rho_holder, antenna_phi_holder); //fill all of the temporary variables for this antenna
			for(int coordinate_iter=0; coordinate_iter<3; coordinate_iter++){ //for every coordinate, transfer the value of the temporary variable to the permanent holder
				antenna_positions[antenna_iter][coordinate_iter]=antenna_position_temporary_holder[coordinate_iter];
			}
		antenna_rho[antenna_iter]=antenna_rho_holder; //assign the temporary value to the correct permanent slot for rho
		antenna_phi[antenna_iter]=antenna_phi_holder; //assign the temporary value to the correct permanent slot for phi
	}//done with getting all of the antenna locations
	...
	const int num_wanted_for_template=40;
	
	int num_found=0;
	
	bool waveforms=false;
	bool waveforms2=true;
	bool coherentsums=false;
	bool coherentsums2=true;

	
	for(Int_t event=0;event<numEntries;event++) { //loop over all events
		cout<<"event "<<event<<endl;
     
		if(event%10==0) cout<<"On event "<<event<<endl;
		...
		if( //2==2 //make a whole bunch of cuts
			...
			&&
			(num_found<num_wanted_for_template)
		){ //if the event passes all cuts, do something with it
			
			double sun_angle = 13.3;
			double size=0.08;
			
			if(waveforms){
				TCanvas *waveform_canvas = new TCanvas("","",2000,1000);
				waveform_canvas->Divide(4,2);
				waveform_canvas->cd(1);
					TGraph *chan2 =realIcrrEvPtr->getGraphFromRFChan(2);
					TGraph *chan2interp = FFTtools::getInterpolatedGraph(chan2,0.5);
					chan2interp -> Draw("AL");
					chan2interp -> GetXaxis()->SetTitle("Time (ns)");
					chan2interp -> GetYaxis()->SetTitle("Voltage (mv)");
					chan2interp -> GetYaxis()->SetTitleOffset(1.2);
					chan2interp->SetTitle("");
					chan2interp->GetXaxis()->SetLabelSize(size);
					chan2interp->GetYaxis()->SetLabelSize(size);
					chan2interp->GetXaxis()->SetTitleSize(size);
					chan2interp->GetYaxis()->SetTitleSize(size);
					gPad->SetLeftMargin(0.15);
				waveform_canvas->cd(5);
					TGraph *chan2spectrum = simpleVoltageSpectrum(chan2interp);
					chan2spectrum->Draw("AL");
					chan2spectrum->GetXaxis()->SetTitle("Frequency (MHz)");
					chan2spectrum->GetYaxis()->SetTitle("Voltage (mv/MHz)");
					chan2spectrum -> GetYaxis()->SetTitleOffset(1.2);
					chan2spectrum->GetXaxis()->SetLabelSize(size);
					chan2spectrum->GetYaxis()->SetLabelSize(size);
					chan2spectrum->GetXaxis()->SetTitleSize(size);
					chan2spectrum->GetYaxis()->SetTitleSize(size);
					chan2spectrum->SetTitle("");
					gPad->SetLogy();
					gPad->SetLeftMargin(0.15);
				waveform_canvas->cd(2);
					TGraph *chan3 =realIcrrEvPtr->getGraphFromRFChan(3);
					TGraph *chan3interp = FFTtools::getInterpolatedGraph(chan3,0.5);
					chan3interp -> Draw("AL");
					chan3interp -> GetXaxis()->SetTitle("Time (ns)");
					chan3interp -> GetYaxis()->SetTitle("Voltage (mv)");
					chan3interp -> GetYaxis()->SetTitleOffset(1.2);
					chan3interp->GetXaxis()->SetLabelSize(size);
					chan3interp->GetYaxis()->SetLabelSize(size);
					chan3interp->GetXaxis()->SetTitleSize(size);
					chan3interp->GetYaxis()->SetTitleSize(size);
					chan3interp->SetTitle("");
					gPad->SetLeftMargin(0.15);
				waveform_canvas->cd(6);
					TGraph *chan3spectrum = simpleVoltageSpectrum(chan3interp);
					chan3spectrum->Draw("AL");
					chan3spectrum->GetXaxis()->SetTitle("Frequency (MHz)");
					chan3spectrum->GetYaxis()->SetTitle("Voltage (mv/MHz)");
					chan3spectrum -> GetYaxis()->SetTitleOffset(1.2);
					chan3spectrum->GetXaxis()->SetLabelSize(size);
					chan3spectrum->GetYaxis()->SetLabelSize(size);
					chan3spectrum->GetXaxis()->SetTitleSize(size);
					chan3spectrum->GetYaxis()->SetTitleSize(size);
					chan3spectrum->SetTitle("");
					gPad->SetLogy();
					gPad->SetLeftMargin(0.15);
				waveform_canvas->cd(3);
					TGraph *chan4 =realIcrrEvPtr->getGraphFromRFChan(4);
					TGraph *chan4interp = FFTtools::getInterpolatedGraph(chan4,0.5);
					chan4interp -> Draw("AL");
					chan4interp -> GetXaxis()->SetTitle("Time (ns)");
					chan4interp -> GetYaxis()->SetTitle("Voltage (mv)");
					chan4interp -> GetYaxis()->SetTitleOffset(1.2);
					chan4interp->GetXaxis()->SetLabelSize(size);
					chan4interp->GetYaxis()->SetLabelSize(size);
					chan4interp->GetXaxis()->SetTitleSize(size);
					chan4interp->GetYaxis()->SetTitleSize(size);
					chan4interp->SetTitle("");
					gPad->SetLeftMargin(0.15);
				waveform_canvas->cd(7);
					TGraph *chan4spectrum = simpleVoltageSpectrum(chan4interp);
					chan4spectrum->Draw("AL");
					chan4spectrum->GetXaxis()->SetTitle("Frequency (MHz)");
					chan4spectrum->GetYaxis()->SetTitle("Voltage (mv/MHz)");
					chan4spectrum -> GetYaxis()->SetTitleOffset(1.2);
					chan4spectrum->GetXaxis()->SetLabelSize(size);
					chan4spectrum->GetYaxis()->SetLabelSize(size);
					chan4spectrum->GetXaxis()->SetTitleSize(size);
					chan4spectrum->GetYaxis()->SetTitleSize(size);
					chan4spectrum->SetTitle("");
					gPad->SetLogy();
					gPad->SetLeftMargin(0.15);
				waveform_canvas->cd(4);
					TGraph *chan6 =realIcrrEvPtr->getGraphFromRFChan(6);
					TGraph *chan6interp = FFTtools::getInterpolatedGraph(chan6,0.5);
					chan6interp -> Draw("AL");
					chan6interp -> GetXaxis()->SetTitle("Time (ns)");
					chan6interp -> GetYaxis()->SetTitle("Voltage (mv)");
					chan6interp -> GetYaxis()->SetTitleOffset(1.2);
					chan6interp->GetXaxis()->SetLabelSize(size);
					chan6interp->GetYaxis()->SetLabelSize(size);
					chan6interp->GetXaxis()->SetTitleSize(size);
					chan6interp->GetYaxis()->SetTitleSize(size);
					chan6interp->SetTitle("");
					gPad->SetLeftMargin(0.15);
				waveform_canvas->cd(8);
					TGraph *chan6spectrum = simpleVoltageSpectrum(chan6interp);
					chan6spectrum->Draw("AL");
					chan6spectrum->GetXaxis()->SetTitle("Frequency (MHz)");
					chan6spectrum->GetYaxis()->SetTitle("Voltage (mv/MHz)");
					chan6spectrum -> GetYaxis()->SetTitleOffset(1.2);
					chan6spectrum->GetXaxis()->SetLabelSize(size);
					chan6spectrum->GetYaxis()->SetLabelSize(size);
					chan6spectrum->GetXaxis()->SetTitleSize(size);
					chan6spectrum->GetYaxis()->SetTitleSize(size);
					chan6spectrum->SetTitle("");
					gPad->SetLogy();
					gPad->SetLeftMargin(0.15);
				char save_name[150];
				sprintf(save_name,"/home/clark.2668/workspace/TrunkSolarFlares/results/today/April_Flare_FourVerticalWaveforms_Event%d.pdf",event);
				waveform_canvas->SaveAs(save_name);
				delete waveform_canvas;
				delete chan2;
				delete chan2interp;
				delete chan2spectrum;
				delete chan3;
				delete chan3interp;
				delete chan3spectrum;
				delete chan4;
				delete chan4interp;
				delete chan4spectrum;
				delete chan6;
				delete chan6interp;
				delete chan6spectrum;
			}
			
			if(waveforms2){
				TCanvas *waveform_canvas = new TCanvas("","",1100,850);
				waveform_canvas->Divide(2,2);
				waveform_canvas->cd(1);
					TGraph *chan2 =realIcrrEvPtr->getGraphFromRFChan(0);
					TGraph *chan2interp = FFTtools::getInterpolatedGraph(chan2,0.5);
					chan2interp -> Draw("AL");
					chan2interp -> GetXaxis()->SetTitle("Time (ns)");
					chan2interp -> GetYaxis()->SetTitle("Voltage (mv)");
				...
				waveform_canvas->cd(3);
					...
					gPad->SetBottomMargin(0.2);
				waveform_canvas->cd(2);
					TGraph *chan3 =realIcrrEvPtr->getGraphFromRFChan(2);
					TGraph *chan3interp = FFTtools::getInterpolatedGraph(chan3,0.5);
					chan3interp -> Draw("AL");
					chan3interp -> GetXaxis()->SetTitle("Time (ns)");
					chan3interp -> GetYaxis()->SetTitle("Voltage (mv)");
			...
			if(coherentsums){
				//TGraph *coherentSumPlaneWave = makeCoherentSum(realIcrrEvPtr, antenna_1_number, num_antennas_used, PeakPhi_value, PeakTheta_value, antenna_positions, antenna_pol,antenna_rho, antenna_phi); //get the coherent sum, version to debug number of contributing waveforms per bin
				TGraph *coherentSumRayTrace = theCorrelator->makeCoherentSum(realIcrrEvPtr,antenna_1_number,PeakPhi_value,PeakTheta_value);
				...
				canvas->cd(1);
				coherentSumRayTrace->Draw("AL");
				TGraph *coherentSumRayTraceSpectrum = simpleVoltageSpectrum(coherentSumRayTrace);
				canvas->cd(4);
				coherentSumRayTraceSpectrum->Draw("AL");
				coherentSumRayTraceSpectrum->GetXaxis()->SetTitle("Frequency (MHz)");
				coherentSumRayTraceSpectrum->GetYaxis()->SetTitle("Voltage (mv)");
				coherentSumRayTraceSpectrum -> GetYaxis()->SetTitleOffset(1.5);
				canvas->cd(2);
				coherentSumRayTraceH->Draw("AL");
				TGraph *coherentSumRayTraceSpectrumH = simpleVoltageSpectrum(coherentSumRayTraceH);
				canvas->cd(5);
				coherentSumRayTraceSpectrumH->Draw("AL");
				coherentSumRayTraceSpectrumH->GetXaxis()->SetTitle("Frequency (MHz)");
				coherentSumRayTraceSpectrumH->GetYaxis()->SetTitle("Voltage (mv)");
				coherentSumRayTraceSpectrumH -> GetYaxis()->SetTitleOffset(1.5);
				canvas->cd(6);
				TGraph *correlation = FFTtools::getCorrelationGraph(coherentSumRayTrace,coherentSumRayTraceH);
				correlation->Draw("AL");
				char save_name[150];
				sprintf(save_name,"/home/clark.2668/workspace/TrunkSolarFlares/results/today/April_CoherentSums_WaveformsandSpectraCorrelation_Event%d.pdf",event);
				canvas->SaveAs(save_name);
				delete canvas;
				delete coherentSumRayTrace;
				delete coherentSumRayTraceSpectrum;
			}
			
			if(coherentsums2){
				//TGraph *coherentSumPlaneWave = makeCoherentSum(realIcrrEvPtr, antenna_1_number, num_antennas_used, PeakPhi_value, PeakTheta_value, antenna_positions, antenna_pol,antenna_rho, antenna_phi); //get the coherent sum, version to debug number of contributing waveforms per bin
				TGraph *coherentSumRayTrace = theCorrelator->makeCoherentSum(realIcrrEvPtr,antenna_1_number,PeakPhi_value,PeakTheta_value);
				TCanvas *canvas = new TCanvas("","",1100,900);
				canvas->Divide(1,2);
				canvas->cd(1);
				coherentSumRayTrace->Draw("AL");
				coherentSumRayTrace->SetLineWidth(2);
				coherentSumRayTrace->GetXaxis()->SetRangeUser(-250,120);
				coherentSumRayTrace->GetXaxis()->SetTitle("Time (ns)");
				coherentSumRayTrace->GetYaxis()->SetTitleOffset(0.6);
				coherentSumRayTrace->GetXaxis()->SetTitleSize(0.08);
				coherentSumRayTrace->GetYaxis()->SetTitleSize(0.08);
				coherentSumRayTrace->GetXaxis()->SetLabelSize(0.08);
				coherentSumRayTrace->GetYaxis()->SetLabelSize(0.08);
				coherentSumRayTrace->GetYaxis()->SetTitle("Time (ns)");
				coherentSumRayTrace->GetYaxis()->SetTitle("Voltage (mV)");
				gPad->SetBottomMargin(0.17);
				coherentSumRayTrace -> SetTitle("");
				canvas->cd(2);
				TGraph *coherentSumRayTraceSpectrum = simpleVoltageSpectrum(coherentSumRayTrace);
				coherentSumRayTraceSpectrum->Draw("AL");
				coherentSumRayTraceSpectrum->SetLineWidth(2);
				coherentSumRayTraceSpectrum->GetYaxis()->SetTitleOffset(0.6);
				coherentSumRayTraceSpectrum->GetXaxis()->SetTitleSize(0.08);
				coherentSumRayTraceSpectrum->GetYaxis()->SetTitleSize(0.08);
				coherentSumRayTraceSpectrum->GetXaxis()->SetLabelSize(0.08);
				coherentSumRayTraceSpectrum->GetYaxis()->SetLabelSize(0.08);
				coherentSumRayTraceSpectrum->GetXaxis()->SetTitle("Frequency (MHz)");
				coherentSumRayTraceSpectrum->GetYaxis()->SetTitle("Spectrum (mV/MHz)");
				coherentSumRayTraceSpectrum -> SetTitle("");
				gPad->SetLogy();
				gPad->SetBottomMargin(0.17);
				char save_name[150];
				sprintf(save_name,"/home/clark.2668/workspace/TrunkSolarFlares/results/today/April_CoherentSums_WaveformsandSpectra_Event%d.pdf",event);
				canvas->SaveAs(save_name);
				delete canvas;
				delete coherentSumRayTrace;
				delete coherentSumRayTraceSpectrum;
...
TGraph *applySpectra(TGraph *input, TGraph *response){
	...
	int inputN = input->GetN();
	//int responseN = response->GetN();
	//if(inputN!=responseN) {cout<<"not the same size!"<<endl; return NULL;}
	
	Double_t *newY = new Double_t [inputN]; //the new Y values
	Double_t *xValsinput = input->GetX();
	...
	for(int i=0; i<inputN; i++){
		input->GetPoint(i,x,start);
		modify = response->Eval(xValsinput[i]); //get the response at that frequency
		double convolution = start*modify;
		if(isnan(convolution)) convolution=0;
		newY[i]=convolution;
	}
	
	TGraph *output = new TGraph(inputN,xValsinput,newY);
	//delete [] newY;
	return output;

}

void Brian_GetFresnel_spol(double i_ang, double n1, double n2, double &r, double &t){ //"s-polarized" light
	double in = TMath::DegToRad()*i_ang;
	double out = asin( n1/n2 * sin(in));
	
	double numerator = (n1 * cos(in)) - (n2 * cos(out));
	double denominator = (n1 * cos(in)) + (n2 * cos(out));
	double ratio = numerator/denominator;
	double magnitude = abs(ratio);
	double square = pow(magnitude,2);
	
	t = sqrt(1.-square);
	r=sqrt(square);
}

void Brian_GetFresnel_ppol(double i_ang, double n1, double n2, double &r, double &t){ //"s-polarized" light
	double in = TMath::DegToRad()*i_ang;
	double out = asin( n1/n2 * sin(in));
	
	double numerator = (n1 * cos(out)) - (n2 * cos(in));
	double denominator = (n1 * cos(out)) + (n2 * cos(in));
	double ratio = numerator/denominator;
	double magnitude = abs(ratio);
	double square = pow(magnitude,2);
...
void spectra_ratio_mapping(int input, int & output1, int & output2){
     switch(input){
          case 8: {output1 = 3; output2 = 0; break;}
          case 9: {output1 = 2; output2 = 5; break;}
          case 10: {output1 = 4; output2 = 7; break;}
          case 11: {output1 = 6; output2 = 1; break;}
     }
}



//a function that will return grNum/grDen
//this is almost identical to the FFTtools function
//just hardened a bit so that zero's in the denominator don't mess the ratios up
TGraph *DivideGraphs(TGraph *grNum, TGraph *grDen)
{
     Int_t N1=grNum->GetN();
     Int_t N2=grDen->GetN();
     if(N1!=N2) return NULL; //check to see if they are the same size, if not, get out
     
     Double_t *newY = new Double_t [N1]; //make an array of this size
     Double_t *xVals=grNum->GetX(); //get the X values
     Double_t x=0.;
     Double_t yA=0.;
     Double_t yB=0.;
     for(int i=0;i<N1;i++) { //iterate over all of the frequencies in the spectra
          grNum->GetPoint(i,x,yA);
          grDen->GetPoint(i,x,yB);
          double ratio = yA/yB;
          if(isnan(ratio)) ratio=0; //if that ratio is a nan set it to zero; basically I'm checking for divison by zero
          newY[i] = ratio;
     }
...
TGraph* simpleVoltageSpectrum(TGraph *grPtr) {
	...
	Int_t maxSamps = 512; //the number of samples to be in the final waveform; here I'm using 2^9 for safety	
	Int_t interpNumSamps=grPtr->GetN();
	Double_t * interpXVals = grPtr->GetX();
	Double_t * interpYVals = grPtr->GetY();
	if(interpNumSamps>512) maxSamps=1024; //if it's bigger th an 2^9, go to 2^10
	Double_t paddedXVals[maxSamps];
	Double_t paddedYVals[maxSamps];
	...
	//delete grPtr;
	grPtr= new TGraph(maxSamps,paddedXVals,paddedYVals);
		
	double *oldY=grPtr->GetY();
	const int oldLength=grPtr->GetN();
	FFTWComplex *theFFT = new FFTWComplex;
	theFFT=FFTtools::doFFT(oldLength,oldY);
	const int fftLength=((grPtr->GetN())/2)+1;
		
	//The time domain answer, ie, the waveform
	double *newX=grPtr->GetX();
	int newLength=grPtr->GetN();
	
	//The frequency domain answer, ie, the spectra
	int length=maxSamps;
	double deltaT=(newX[1]-newX[0])*1.e-9; //ns
	double deltaF=1/(deltaT*length); //Hz
	deltaF*=1e-6; //MHz
	double SpectraY[fftLength];
	double SpectraX[fftLength];
	double tempF=0;
	for(int i=0;i<fftLength;i++) {
		SpectraY[i] = FFTtools::getAbs(theFFT[i]);// /(double) numGraphs;
		SpectraX[i]=tempF;
		tempF+=deltaF;
	}
	
	TGraph *output = new TGraph(fftLength,SpectraX,SpectraY); //the spectra. Isn't my naming everything "Outputgraph" a good idea
...
void simpleVoltageSpectrumwithFresnel(TGraph *grPtr, TGraph * (&OutputGraphs)[2], double incident_angle, bool apply_fresnel, int pol) {
	...
	double para_t=1.;
	if(apply_fresnel){
		Brian_GetFresnel_spol(incident_angle,1.0,index,perp_r,perp_t);
		Brian_GetFresnel_ppol(incident_angle,1.0,index,para_r,para_t);
	}
	for(int j=0; j<oldLength; j++){
		if(pol==0) oldY[j]/=para_t; //cout<<"perp t is "<<perp_t<<endl;
		else if(pol==1) oldY[j]/=perp_t; //cout<<"para t is "<<para_t<<endl;
	}
	theFFT=FFTtools::doFFT(oldLength,oldY);
	...
	double *newY=FFTtools::doInvFFT(newLength,theFFT);
	OutputGraphs[0]=new TGraph(newLength,newX,newY); //the waveform
	OutputGraphs[1]=new TGraph(fftLength,SpectraX,SpectraY); //the spectra. Isn't my naming everything "Outputgraph" a good idea
	//The above snark is for my future self to know you're not crazy when you read this later
...
void AverageSpectra(Int_t numGraphs, TGraph **grPtr, TGraph * (&OutputGraphs)[2], vector<double>& incident_angles, bool apply_fresnel, int pol) {
	...
	//Ryan made a very costly (2 work days!!) error in this code
	//He didn't pad the waveforms to be a power of 2, so the FT was all screwy, putting NANs where the shouldn't have been
	//So I had to fix it
	Double_t interpolationStep=0.5; //this is the interpolation distance for the waveform
	Int_t maxSamps = 512; //the number of samples to be in the final waveform; here I'm using 2^9 for safety
	if(grPtr[0]->GetN()>512) maxSamps=1024; //if it's bigger th an 2^9, go to 2^10

     
	
	for(int j=0; j<numGraphs; j++){
		Int_t interpNumSamps = grPtr[j]->GetN();
		Double_t * interpXVals = grPtr[j]->GetX();
		Double_t * interpYVals = grPtr[j]->GetY();
		Double_t paddedXVals[maxSamps];
		Double_t paddedYVals[maxSamps];
		for(int i=0; i<maxSamps; i++){ //loop over all of the data points in the final set
			if(i<interpNumSamps){ //if the iterator is less than the number of samples in the waveform
		...
		delete grPtr[j];
		grPtr[j]= new TGraph(maxSamps,paddedXVals,paddedYVals);
	...
	for(int i=0;i<numGraphs;i++) {
		...
		const int oldLength=grPtr[i]->GetN();
		...
		double para_t=1.;
		if(apply_fresnel){
			Brian_GetFresnel_spol(incident_angles[i],1.0,index,perp_r,perp_t);
			Brian_GetFresnel_ppol(incident_angles[i],1.0,index,para_r,para_t);
		}
		...
		theFFTs[i]=FFTtools::doFFT(oldLength,oldY);
	}
	

	
	const int fftLength=((grPtr[0]->GetN())/2)+1;
	FFTWComplex *combinedFFT = new FFTWComplex [fftLength];
	
	
	for(int i=0;i<fftLength;i++) {
		double tempAbs0=FFTtools::getAbs(theFFTs[0][i]);
		double tempTotAbs=tempAbs0;
		for(int grNum=1;grNum<numGraphs;grNum++) {
			tempTotAbs+=FFTtools::getAbs(theFFTs[grNum][i]);
		}
		...
		//as an fyi, the reason for the division by tempAbs0 is because theFFTs[0][i].re IS tempAbs0, it's just used as a base, so it has to be gotten rid of
		//this seems dumb, but it seems to work, so who am I to argue...
	...
	double tempF=0;
	for(int i=0;i<fftLength;i++) {
		SpectraY[i] = FFTtools::getAbs(combinedFFT[i]);// /(double) numGraphs;
		SpectraX[i]=tempF;
		tempF+=deltaF;
	}
	
	/*
	double quick_integral_voltage =0.;
	for(int j=0; j<grPtr[0]->GetN(); j++){
		quick_integral_voltage+=(grPtr[0]->GetY()[j]*grPtr[0]->GetY()[j]);
	}
	double quick_integral_frequency=0.;
	for(int j=0; j<fftLength; j++){
		quick_integral_frequency+=(FFTtools::getAbs(theFFTs[0][j])*FFTtools::getAbs(theFFTs[0][j])/((double) fftLength));
	}
	cout<<"voltage integral "<<quick_integral_voltage<<endl;
	cout<<"frequency integral "<<quick_integral_frequency<<endl;
	*/
	
	double *newY=FFTtools::doInvFFT(newLength,combinedFFT);
	OutputGraphs[0]=new TGraph(newLength,newX,newY);; //the waveform
	//OutputGraphs[0]=new TGraph(newLength,newX,newY);; //the waveform
	OutputGraphs[1]=new TGraph(fftLength,SpectraX,SpectraY);; //the spectra. Isn't my naming everything "Outputgraph" a good idea
	//The above snark is for my future self to know you're not crazy when you read this later
...
void averageTemplates(TGraph *in[n_chan][num_wanted_for_template], TGraph *out[n_chan])
{
	for(Int_t i=0;i<n_chan;++i) out[i]=FFTtools::interpolateCorrelateAndAverage(0.5, num_wanted_for_template,in[i]); 
}


//code to figure out if this is an ICL rooftop pulser event
bool isICLPulserEvent(RawIcrrStationEvent *evPtr){
     return false; //just something so the code will compile
}
	...
	*/
	...
	for(int antenna_iterator=0; antenna_iterator<num_antennas_used; antenna_iterator++){ //begin the loop over all of the other antennas							
		if(antenna_iterator==antenna_1_number){ //because we're using pushback, this antenna should get shoved in the right place in the line
			...
			cout<<"Time delay "<<antenna_1_number<<" and "<<antenna_iterator<<" is "<<time_delay_2<<endl;

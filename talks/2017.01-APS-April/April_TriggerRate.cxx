...
////  analysis.cxx 
////
////    Nov 2016,  clark.2668@osu.edu
////	Code to compare the measured peak to predicted location
////    This particular version of the code assumes the input files already contain the pre-made interferometric maps; this allows for must faster analysis, not needing to make maps
////////////////////////////////////////////////////////////////////////////////
...
int main(int argc, char **argv)
{
	...
	int num_found_max=5; //a variable to hold the maximum number of waveforms you want to allow in the first input file loop
	
	TString dataPath("/home/clark.2668/workspace/TrunkSolarFlares/source_AraRoot_Trunk/"); //the location of the files	
	...
	TProfile *triggerrate = new TProfile("","",100,0,100,0,10000);                                                                                                                                                                
	...
	for(Int_t event=0;event<numEntries;event++) { //loop over all events
		...
		{ //for determining trigger rate, loop over everything
			...
			triggerRate.push_back(rawIcrrEvPtr->hk.sclGlobal);
			timeStamp.push_back( ((((double)hour)-1.)*60.) + (((double) min) + (((double) sec)/60.))); //push back the time after 1 AM in minutes
			double time = ((((double)hour)-1.)*60.) + ((double) min);
			double rate = rawIcrrEvPtr->hk.sclGlobal;
			triggerrate->Fill( time, rate);
			...
			if(num_found == 0) cout<<"the first event is "<<event<<endl;
			if(num_found%50 ==0) cout<<"on event "<<num_found<<endl;
			
		}//end outer if statement about time constraints	
	...
	TGraph *trigger_rate = new TGraph(num_found, &timeStamp[0], &triggerRate[0]);
	
	TCanvas *trigger_rate_canvas = new TCanvas("","",1100,850);
	trigger_rate->Draw("A*");
	trigger_rate->SetMarkerStyle(kDot);
	trigger_rate_canvas->SetLogy();
	triggerrate->Draw("same");
	triggerrate->SetLineColor(kRed);
	TLine l1(44,1,44,1000);
		l1.SetLineWidth(3);
		l1.Draw("same");
		l1.SetLineColor(kBlue);
	TLine l2(54,1,54,1000);
		l2.SetLineWidth(3);
		l2.Draw("same");
		l2.SetLineColor(kGreen);
	TLine l3(56,1,56,1000);
		l3.SetLineWidth(3);
		l3.Draw("same");
		l3.SetLineColor(kRed);
	trigger_rate_canvas->Update();
	trigger_rate->SetTitle("");
	trigger_rate->GetXaxis()->SetTitle("Minutes after 1 GMT");
	trigger_rate->GetYaxis()->SetTitle("Trigger Rate (Hz)");
	//trigger_rate->GetYaxis()->SetTitleOffset(1.5);
	trigger_rate_canvas->Update();
	sprintf(save_title,"/home/clark.2668/workspace/TrunkSolarFlares/results/today/AprilMeeting_trigger_rate.pdf",year_now,month_now,day_now);
	trigger_rate_canvas->SaveAs(save_title);
	delete trigger_rate;
	delete trigger_rate_canvas;
	
	TCanvas *trigger_rate_canvas_2 = new TCanvas("","",1100,850);
	triggerrate->Draw();
	triggerrate->SetLineWidth(2);
	triggerrate->GetYaxis()->SetLabelSize(0.05);
	triggerrate->GetXaxis()->SetLabelSize(0.05);
	triggerrate->GetYaxis()->SetTitleSize(0.05);
	triggerrate->GetXaxis()->SetTitleSize(0.05);
	gPad->SetLogy();
	//gStyle->SetPadLeftMargin(0.01);
	//gStyle->SetPadRightMargin(0.01);
	triggerrate->SetLineColor(kBlack);
	trigger_rate_canvas_2->Update();
	triggerrate->SetTitle("");
	triggerrate->GetXaxis()->SetTitle("Minutes after 1 GMT");
	triggerrate->GetYaxis()->SetTitle("Trigger Rate (Hz)");
	trigger_rate_canvas_2->Update();
	sprintf(save_title,"/home/clark.2668/workspace/TrunkSolarFlares/results/today/AprilMeeting_TriggerRate_Histogram.pdf",year_now,month_now,day_now);
	trigger_rate_canvas_2->SaveAs(save_title);
	delete trigger_rate_canvas_2;
	delete triggerrate;
...
void GetCorrMapPeakAngle_1deg_SolarAnalysis( TH2D *theCorrMap_input, int &peaktheta, int &peakphi, double &peakvalue, int eventType) {
	...
	if(eventType==0){ //a flare event
		for (int theta=130; theta<=180; theta++) { //right now, only look at peaks above theta = 20
